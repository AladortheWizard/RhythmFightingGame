...
public class InteractionsManager : MonoBehaviour
{
    ...
    { 
        if (attacker == "p1")
        {
            if (ComboManagerP2.instance.state != "neutral" && strongAgainst == "neutral")
            {
        ...
        else if (attacker == "p2")
        {
           ...
           else if (strongAgainst == ComboManager.instance.state || ComboManager.instance.state == "stun" || ComboManager.instance.state == "neutral")
            {
                healthBar.transform.localScale = new Vector3(healthBar.transform.localScale.x - (dmg * .01f), 1, 1);
